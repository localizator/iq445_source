...
u32 pinSet[3][8] = {
    ...
    if (On) {
        ...
        if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_IMX105_MIPI_RAW,currSensorName))){
            PK_DBG("[CAMERA SENSOR] MINGJI TEST IMX105_MIPI_RAW case\n");
            ...
            PK_DBG("[CAMERA SENSOR] MINGJI TEST OV5647_MIPI_RAW case\n");
            if(TRUE != hwPowerOn(CAMERA_POWER_VCAM_D, VOL_1800,mode_name))//VOL_1500 mingji modify.
            {
                 PK_DBG("[CAMERA SENSOR] Fail to enable digital power\n");
                 //return -EIO;
                 goto _kdCISModulePowerOn_exit_;
            ...
            PK_DBG("[CAMERA SENSOR] MINGJI TEST GENERAL case\n");
            if(TRUE != hwPowerOn(CAMERA_POWER_VCAM_D, VOL_1500,mode_name))//VOL_1500 mingji modify.
            {
                 PK_DBG("[CAMERA SENSOR] Fail to enable digital power\n");
                 //return -EIO;
                 goto _kdCISModulePowerOn_exit_;
