...
extern signed char OV5648_otp_read_wb_group(signed char index);
extern bool OV5648_otp_update_wb(unsigned short golden_rg, unsigned short golden_bg);
...
static UINT32 OV5648_OTP_combination_read(void)
{
    ...
    for(index = 0; index < 3; index++)
    {
        ...
        mid = OV5648MIPI_read_cmos_sensor(address);
        //mid &= 0x7f;
        printk("*********************************************MID = %d\n",mid);
        printk("*********************************************index = %d\n",index);
    ...
    address = 0x3d00;
    for(i=0; i<16;i++) {
        OV5648MIPI_write_cmos_sensor(address + i, 0x00);
    }

    printk("OV5648_OTP_combination_read()!!!!! index==%d\n", index);
    if(index > 2)
        return FALSE;
    else
        return TRUE;
...
/*
    ...
    OV5648MIPI_write_cmos_sensor(0x0100, 0x01);
    mdelay(50);
    OV5648_otp_update_wb(0x011E, 0x018C);
    SENSORDB("This is sunny module!!!!!!!!!!!!!\n");
*/
...
UINT32 OV5648MIPIOpen(void)
{
    ...
    SENSORDB("OV5648MIPIOpen sensor_id = 0x%x ", sensor_id);
...
UINT32 OV5648GetSensorID(UINT32 *sensorID) 
{
    ...
    *sensorID=((OV5648MIPI_read_cmos_sensor(0x300A) << 8) | OV5648MIPI_read_cmos_sensor(0x300B));   
    ...
    if (*sensorID == OV5648MIPI_SENSOR_ID) {
        //check otp mid
        if (OV5648_OTP_combination_read()){
            printk("[OV5648GetSensorID],this is truly module,return.\n");
            *sensorID = 0xFFFFFFFF;
            return ERROR_SENSOR_CONNECT_FAIL;
        }
        else{
            printk("[OV5648GetSensorID],this is sunny module,go on.\n");
        }
    }
        
    if (*sensorID != OV5648MIPI_SENSOR_ID) {
//Try again with invert PWD Pin config
	printk("OV5648GetSensorID Try again with PWD High \n");

	//mt_set_gpio_out(GPIO_CAMERA_CMPDN_PIN,GPIO_OUT_ONE);
	set_maincam_powerdown(GPIO_OUT_ONE);
	mdelay(5);
	*sensorID=((OV5648MIPI_read_cmos_sensor(0x300A) << 8) | OV5648MIPI_read_cmos_sensor(0x300B));	
	printk("OV5648MIPIOpen, sensor_id:%x \n",*sensorID);
