...
#ifdef CTP_SIMULATE_PS  //edit by Magnum 2013-5-17
...
static u8 ft6x06_get_ps(void);
static void ft6x06_ctp_convert(char mode,bool force);
#endif
...
static const struct i2c_device_id ft5316_i2c_id[] = {{"FT6x06",0},{}};
static struct i2c_board_info __initdata ft5316_i2c_tpd={ I2C_BOARD_INFO("FT6x06", (TPD_SLAVE_ADDR>>1))};
...
static struct tpd_driver_t tpd_device_driver = {
		...
		.ctp_get_ps =  ft6x06_get_ps,
		.ctp_convert =ft6x06_ctp_convert,
#endif
};

struct i2c_driver tpd_i2c_driver = {
    .driver = {
	    .name = "FT6x06",//.name = TPD_DEVICE,
    },    
    .probe = tpd_i2c_probe,
...
/*	if(force==false)
	...
	if(DISABLE_CTP_PS == mode)
	{
		...
		ret = i2c_smbus_write_byte_data(ft5316_i2c_client, 0xB0, 0);
		buffer = i2c_smbus_read_byte_data(ft5316_i2c_client, 0xB0);
		TPD_DEBUG("Disable CTP PS B0 == 0x%x \n",buffer);
		if(ret < 0)
			TPD_DEBUG("Disable CTP PS Error \n");
	}
	else if(ENABLE_CTP_PS == mode)
	{
		TPD_DEBUG("ENABLE_CTP_PS mode=%d\n", mode);
		ps_mode = 1;
		ret = i2c_smbus_write_byte_data(ft5316_i2c_client, 0xB0, 1);
		buffer = i2c_smbus_read_byte_data(ft5316_i2c_client, 0xB0);
		TPD_DEBUG("Enable CTP PS B0 == 0x%x \n",buffer);
		if(ret < 0)
			TPD_DEBUG("Enable CTP PS Error \n");
	}
}
...
static void tpd_down(int x, int y, int id) {
    if(tpd_status)
    {
	 ...
	 if(RECOVERY_BOOT != get_boot_mode())
	   {
	   	    TPD_DEBUG("tpd_down, report abs ,x == %d, y == %d\n",x,y);
...
int tpd_gettouchinfo(struct touch_info *cinfo, struct touch_info *sinfo) 
{
	...
	ret = i2c_master_recv(ft5316_i2c_client, &buffer[0], 7);
...
//	ret = i2c_master_recv(i2c_client, &buffer[0],8 );
	...
	for(i;i<4;i++){
	 ...
	 #ifdef CTP_SIMULATE_PS
				if(buffer[1] == 0xc0) // close to
				{
				    TPD_DEBUG("slaver report i am close...\n");
					ps_data = 0;
				}
				else if(buffer[1] == 0xe0) // leave
				{
...
//Ivan added for testing
	...
	if(tpd_load_status&&(ps_data_pre!=ps_data)){
