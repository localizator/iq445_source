...
bool get_device_info(u16 id, u32 ext_id, flashdev_info * pdevinfo)
{
    ...
    for (index = 0; gen_FlashTable[index].id != 0; index++)
    {
	    ...
	    pdevinfo->sparesize = gen_FlashTable[index].sparesize;
            ...
            MSG(INFO, "NAND INFO: totalsize=%d MB,blocksize=%d KB, pagesize=%d Byte ,iowidth=%d bit name=%s \n", pdevinfo->blocksize, pdevinfo->pagesize, pdevinfo->totalsize, pdevinfo->iowidth,
                pdevinfo->devciename);
            break;
        }
    }
...
{
    int i, j, busw, id, index;
    u8 id_arry[NAND_MAX_ID];
    u32 ext_id;
    u32 spare_bit;
    u32 spare_per_sec;
    u32 ecc_bit;
    ...
    {
        id_arry[index] = nand_read_byte();
        MSG(INFO, " %x", id_arry[index]);
    }
    MSG(INFO, "\n ");
    id = (id_arry[0] << 8) | id_arry[1];
    ext_id = id_arry[2] << 16 | id_arry[3] << 8 | id_arry[4];
    ...
    nand->id_length = NAND_MAX_ID;
    for (index = 0; index < NAND_MAX_ID; index++)
    {
        nand->id[index] = id_arry[index];
    }
	...
	if(!devinfo.sparesize){
    		nand->oobsize = (8 << ((id_arry[3] >> 2) & 0x01)) * (nand->oobblock / 512); //FIX ME ,kai
	}else{
