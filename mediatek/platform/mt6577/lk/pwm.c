...
void mt_pwm_power_on(U32 pwm_no)
{
	if ( pwm_no == PWM3 || pwm_no == PWM4 || pwm_no == PWM5) {
		PWM_CLR_BITS(PWM_CLOCK, (1<<0x5));  //enable clock 
		PWMDBG("enable_clock PWM345\n");
	}
	if ( pwm_no == PWM0) {
		PWM_CLR_BITS(PWM_CLOCK, (1<<0x2));  //enable clock 
		PWMDBG("enable_clock PWM0\n");
	}
	if ( pwm_no == PWM1) {
		PWM_CLR_BITS(PWM_CLOCK, (1<<0x3));  //enable clock 
		PWMDBG("enable_clock PWM1\n");
	}
	if ( pwm_no == PWM2) {
		PWM_CLR_BITS(PWM_CLOCK, (1<<0x4));  //enable clock 
		PWMDBG("enable_clock PWM2\n");
	}
	if ( pwm_no == PWM6) {
		PWM_CLR_BITS(PWM_CLOCK, (1<<0x6));  //enable clock 
		PWMDBG("enable_clock PWM6\n");
	}
	mdelay(100);
}
...
{
	if ( pwm_no == PWM0) {
		PWM_SET_BITS(PWM_CLOCK, (1<<0x2));  //disable clock 
		PWMDBG("disable_clock PWM0\n");
	}
	if ( pwm_no == PWM1) {
		PWM_SET_BITS(PWM_CLOCK, (1<<0x3));  //disable clock 
		PWMDBG("disable_clock PWM1\n");
	}
	if ( pwm_no == PWM2) {
		PWM_SET_BITS(PWM_CLOCK, (1<<0x4));  //disable clock 
		PWMDBG("disable_clock PWM2\n");
	}
	if ( pwm_no == PWM4 || pwm_no == PWM5 || pwm_no == PWM3) {
		PWM_SET_BITS(PWM_CLOCK, (1<<0x5));  //disable clock 
		PWMDBG("disable_clock PWM345\n");
	}
	if ( pwm_no == PWM6) {
		PWM_SET_BITS(PWM_CLOCK, (1<<0x6));  //disable clock 
		PWMDBG("disable_clock PWM6\n");
	}
}
...
S32 pwm_set_spec_config(struct pwm_spec_config *conf)
{
	...
	if ( (( conf->pwm_no == PWM4 || conf->pwm_no == PWM5||conf->pwm_no == PWM3 )
	...
	switch (conf->mode ) {
		...
		case PWM_MODE_DELAY:
			...
			if ( conf->pwm_mode.PWM_MODE_DELAY_REGS.PWM3_DELAY_DUR <0 ||conf->pwm_mode.PWM_MODE_DELAY_REGS.PWM3_DELAY_DUR >= (1<<17) ||
					conf->pwm_mode.PWM_MODE_DELAY_REGS.PWM4_DELAY_DUR < 0|| conf->pwm_mode.PWM_MODE_DELAY_REGS.PWM4_DELAY_DUR >= (1<<17) ||
			...
			mt_set_pwm_delay_duration(PWM3_DELAY, conf->pwm_mode.PWM_MODE_DELAY_REGS.PWM3_DELAY_DUR );
			mt_set_pwm_delay_clock(PWM3_DELAY, conf->pwm_mode.PWM_MODE_DELAY_REGS.PWM3_DELAY_CLK);
...
S32 mt_get_pwm_enable(U32 pwm_no)
{
	...
	if ( pwm_no >= PWM_MAX ) {
		PWMDBG("pwm number is not between PWM0~PWM6.\n");
		return -EEXCESSPWMNO;
	}
