...
SPELayer::SPELayer()
{
	...
	mRecordPGAGain = 16;	//PGA_gain for AGC
	...
	mVoIPPGAGain = 16;	//PGA_gain for AGC
...
bool	SPELayer::SetPGAGain(SPE_MODE mode, long PGA_gain)
{
	...
	ALOGD("SPELayer::SetPGAGain, SPE_MODE=%d, gain=%d",mode,PGA_gain);
...
long	SPELayer::GetPGAGain(SPE_MODE mode)
{
	...
	switch(mode)
	{
		...
		case SPE_MODE_VOIP:
			...
			break;
		default:
			retPGAGain = mSph_Enh_ctrl.PGA_gain;
			break;
	}
...
bool    SPELayer::Start(SPE_MODE mode)  //for VOIP, both uplink/downlink
{
	...
	if(mode == SPE_MODE_REC)
	{
	    ...
	    mSph_Enh_ctrl.PGA_gain = mRecordPGAGain;
	...
	else if(mode == SPE_MODE_VOIP)
	{
	    ...
	    mSph_Enh_ctrl.PGA_gain = mVoIPPGAGain;
...
*/		
	...
	ALOGD("Using:Samplerate = %d, FrameRate=%d,PGAGain=%d, App_table=%x, Fea_Cfg_table=%x",mSph_Enh_ctrl.sample_rate,mSph_Enh_ctrl.frame_rate,mSph_Enh_ctrl.PGA_gain, mSph_Enh_ctrl.App_table,mSph_Enh_ctrl.Fea_Cfg_table);
        ...
        if(dir == UPLINK)
        {
			...
			ret = checkAndCreateDirectory(filename);
		   	...
		   	else
		   	{
	            ...
	            if(mfpOutUL!=NULL)
	            {
	                fwrite(buffer,bytes,1,mfpOutUL);
	            }
	            else
	            {
        ...
        else
        {
			...
			ret = checkAndCreateDirectory(filename);
		   	...
		   	else
		   	{
	            ...
	            if(mfpOutDL!=NULL)
	            {
	                fwrite(buffer,bytes,1,mfpOutDL);
	            }
	            else
	            {
