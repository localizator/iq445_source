                                             ...
                                             int delayMs)
{
    ...
    if (( (device == 0 && AudioSystem::popCount(prevDevice) != 2 ) || device == prevDevice) && !force) {
        ALOGD("setOutputDevice() setting same device %x or null device for output %d", device, output);
        return 0;
    }
#else
    ...
    param1.addInt(String8(AudioParameter::keyRouting), (int)device);
    mpClientInterface->setParameters(output, param1.toString(), delayMs);
