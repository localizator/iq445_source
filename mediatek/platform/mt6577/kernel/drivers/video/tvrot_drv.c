...
#define MT_SYSRAM_BASE_PA 0xC2000000
#define MT_SYSRAM_BASE_VA 0xF2000000
#define MT_SYSRAM_SIZE    0x40000 //256kb
...
#if defined TV_BUFFER_PIPE
    _tvrContext.descAddr = desc_addr - MT_SYSRAM_BASE_PA + MT_SYSRAM_BASE_VA;
#endif
#else
...
{
 #if ENABLE_TVROT_INTERRUPT
    if (request_irq(MT_TV_ROT_IRQ_ID,
        (irq_handler_t)_TVR_InterruptHandler, IRQF_TRIGGER_LOW, "mt-tvrot", NULL) < 0)
    {
...
bool TVR_EnqueueBuffer(UINT32 addr, UINT32* pAddrPrev)
{
    ...
    */
    ...
    ASSERT(MT_SYSRAM_BASE_VA <= descAddr &&
                 descAddr < MT_SYSRAM_BASE_VA + MT_SYSRAM_SIZE);
