...
void device_APC_dom_setup(void)
{
    ...
    reg_write32(MD_DEVAPC1_APC_CON, 0x0);
#ifdef DBG_PRELOADER
    MSG(DOM, "AP Domain Setup (0x%x)\n", reg_read32(AP_DEVAPC1_MAS_DOM));
    MSG(DOM, "MD 0 Domain Setup (0x%x)\n", reg_read32(MD_DEVAPC0_MAS_DOM));
    MSG(DOM, "MD 1 Domain Setup (0x%x)\n", reg_read32(MD_DEVAPC1_MAS_DOM));
    MSG(DOM, "DSP Domain Setup (0x%x)\n", reg_read32(DSP_DEVAPC_MAS_SEC));    
#endif
    reg_set_field(MD_DEVAPC1_MAS_DOM, MD_2G_DOM_CTRL, DOMAIN_FCORE);
    reg_set_field(MD_DEVAPC1_MAS_DOM, MD_3G_DOM_CTRL, DOMAIN_MD);
