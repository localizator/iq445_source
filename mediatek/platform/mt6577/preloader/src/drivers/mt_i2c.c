...
U32 i2c_v1_set_speed (unsigned long clock, I2C_SPD_MODE mode, unsigned long khz)
{
    ...
    print("[i2c_set_speed] Set sclk to %d khz (orig: %d khz)\n", sclk, khz);
    print("[i2c_set_speed] I2C Timing parameter sample_cnt_div(%d),  step_cnt_div(%d)\n", sample_cnt_div, step_cnt_div);
...
U32 i2c_v1_init (void) 
{
    ...
    if( ret_code !=  I2C_OK)
    {
        print("[i2c_init] i2c_v1_set_speed error (%d)\n", ret_code);
        return ret_code;
    }
...
U32 i2c_v1_read(U8 chip, U8 *buffer, int len)
{
    ...
    if (!len) {
        print("[i2c_read] I2C doesn't support len = 0.\n");
    ...
    while (1) 
    {
        ...
        else if ( status & I2C_HS_NACKERR) 
        {
            ...
            print("[i2c_read] transaction NACK error (%x)\n", status);
            break;
        }
        else if ( status & I2C_ACKERR) 
        {
            ...
            print("[i2c_read] transaction ACK error (%x)\n", status);
            break;
        }
        ...
        //else if (gpt4_timeout_tick(start_tick, timeout_tick))  {
            ...
            print("[i2c_read] transaction timeout (%x)\n", status);
            break;
        }		
    }
...
U32 i2c_v1_write (U8 chip, U8 *buffer, int len)
{
    ...
    while (len--) {
        I2C_WRITE_BYTE(*ptr);
        //print("[i2c_write] write byte = 0x%x\n", *ptr);
        ptr++;
    }
    ...
    while (1) {
        ...
        if ( status & I2C_TRANSAC_COMP) {
            ...
            print("[i2c_write] transaction NACK error\n");
            break;
        }
        else if ( status & I2C_ACKERR) {
            ret = 2;
            ret_code = I2C_WRITE_FAIL_ACKERR;			
            print("[i2c_write] transaction ACK error\n");
            break;
        }
        else if (get_timer(tmo) > I2C_TIMEOUT_TH /* ms */ ) {
        //else if (gpt4_timeout_tick(start_tick, timeout_tick)) {
            ...
            print("[i2c_write] transaction timeout\n");
            break;
        }
...
extern U32 mt_get_bus_freq(void);
